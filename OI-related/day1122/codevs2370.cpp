...
ll tree_qjh_query(int l, int r) {
    ...
    if (l == r) return resu;
    resu += slpf.qjh_query(1, 1, tot - 1, locator[h_son[l]], locator[r]);
    return resu;
}
int main() {
    CLEAR(nodeweight);
    ...
    for (int i = 0; i < n - 1; ++i) {
        ...
        v[tempa + 1].push_back(tempb + 1);
        v[tempb + 1].push_back(tempa + 1);
        vw[tempa + 1].push_back(tempc);
        vw[tempb + 1].push_back(tempc);
    }
    head[1] = 1;
    dfs_1(1, 0, 1);
    dfs_2(1, 1);
    ...
    for (int i = 0; i < q; ++i) {
        ...
        printf("%lld\n", tree_qjh_query(tempa + 1, tempb + 1));
    }
}
